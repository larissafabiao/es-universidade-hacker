...
Então('ele deve ter sido salvo no banco de dados') do
  ...
  expect(aluno.nome).to eq('Jou Kuroda')
  expect(aluno.email).to eq('joukrd.usp@usp.br')
end

Então('deverei ver o aluno na página de listagem de alunos') do
  expect(page).to have_content('Jou Kuroda')
  expect(page).to have_content('joukrd.usp@usp.br')
end
